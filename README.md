...
## TDX 到底有哪些路?(2026-09-08 實測)

「TDX 是不是只有高速公路」這件事,文件上看不出來——TDX 確實有一整組以縣市為單位的道路資源,但**餵不餵資料是各縣市政府決定的**。
所以直接打 API 問:`/api/diagnostics/tdx-city`(可加 `?reverse=1`)與 `/api/diagnostics/tdx-highway`。

| 資源 | 結果 |
|---|---|
| 國道即時(目前在收) | 200,有旅行時間 |
| 新竹市 VD(靜態) | 200,**只有 3 個偵測器**,全部在公道五交流道匝道 |
| 新竹市 VD(即時) | 200,**0 筆** |
| 新竹縣 VD(靜態/即時) | 各 2 筆(竹北的自強南路、文興路一段) |
| 新竹市 路段旅行時間(即時) | **404,這個 API 不存在** |
| 新竹市 路段(靜態) | 400 |
| 新竹市 CCTV | 200,**183 支**,涵蓋光復路、介壽路、慈雲路、經國路、中華路、食品路、公道五、埔頂、台68 |
| 省道/快速道路 路段(靜態) | 200,6831 筆(台1線 552、台61 919、台3線 376、台15 78、台1甲 66、台68 14) |
| 省道/快速道路 即時 | 200,6652 筆,欄位是 `SectionID / TravelTime / TravelSpeed / CongestionLevel / DataCollectTime` |
| **台68 即時** | **0 筆**——靜態有 14 段,即時一段都對不到(不是 ID 對不起來:即時資料裡沒有任何台68 的 SectionID) |

三個對這個產品直接有影響的結論:

1. **光復路、關新路、介壽路這類純市區道路,TDX 沒有任何速度或旅行時間資料。** 只有攝影機影像。目前收的國道資料再乾淨,也回答不了「今天光復路塞不塞」。
2. **省道是目前唯一「非國道、而且真的有數字」的來源**,而且欄位形狀跟國道一模一樣(同一張表、同一套基準線數學就能吃)。台1線就是市區的中華路,台3線往竹東。
3. **台68 在靜態路段表裡,但即時資料沒有它**。查的時間是台北時間 00:17,白天是否不同還沒驗證。

**資料品質警告**:省道即時資料裡有大量 sentinel——`CongestionLevel = "-99"`、`TravelSpeed = 0`、`TravelTime` 出現 250560 秒(69 小時)這種值。要接的話必須先濾掉,否則基準線的百分位數會被垃圾整個拉爆。

**還沒查的**:新竹市政府自己的開放資料平台可能有市區路況;商用 API(Google Routes / HERE)一定有,但要錢。
